...
import { getCurrentBoard } from "@/app/actions";

// ... imports ...

export const boardsApi = {
  getCurrent: async (): Promise<VisionBoard> => {
    // Direct Server Action call (Next.js handles the bridge)
    const board = await getCurrentBoard();
    if (board) return board;

    // Fallback to mock if DB is empty or explicitly mocking (optional)
